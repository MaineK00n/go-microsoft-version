...
// Version represents a .NET Framework version (e.g., "4.8.04584.08", "4.8.4465.0")
// Segments may contain leading zeros in the raw data but are compared numerically.
type Version struct {
	Major    int
	Minor    int
	Build    int
...
func NewVersion(ver string) (Version, error) {
	...
	if len(ss) != 4 {
		return Version{}, fmt.Errorf("unexpected .NET Framework version format. expected: %q, actual: %q", "<major>.<minor>.<build>.<revision>", ver)
	}
