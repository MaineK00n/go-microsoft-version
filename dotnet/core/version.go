...
// Version represents a .NET (Core/5+) version (e.g., "8.0.2", "9.0.13")
type Version struct {
	Major int
	Minor int
	Patch int
...
func NewVersion(ver string) (Version, error) {
	...
	if len(ss) != 3 {
		return Version{}, fmt.Errorf("unexpected .NET (Core/5+) version format. expected: %q, actual: %q", "<major>.<minor>.<patch>", ver)
	}
