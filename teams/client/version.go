...
// Version represents a Microsoft Teams version (e.g., "2.10.1", "1.3.0")
type Version struct {
	Major int
	Minor int
	Patch int
...
func NewVersion(ver string) (Version, error) {
	...
	if len(ss) != 3 {
		return Version{}, fmt.Errorf("unexpected Teams version format. expected: %q, actual: %q", "<major>.<minor>.<patch>", ver)
	}
